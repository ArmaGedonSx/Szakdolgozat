...
namespace Chat_Server
{
    ...
    public class Server
    {
        ...
        public Server(ServerInfo info)
        {
            ...
            string FilePath = Database.ServersHistoryFolder + @"/" + Info.SID + ".txt";
        ...
        public bool Connect(User user, string password, IPEndPoint currentEnd)
        {
            if (user != null)
            {
                ...
                if (IsUserConnected(user.Info.Login)) // Ellenőrizzük, hogy már csatlakozott-e a felhasználó
                {
                    return false; // Ha igen, ne engedjük meg a duplikált csatlakozást
                }
                ...
                {
                    user.CurrentServer.Disconnect(user, currentEnd);
        ...
        public void OnConnect(User user, IPEndPoint end)
        {
            ...
            Alert($"{user.Info.Nickname} joined", end);
        ...
        public void OnDisconnect(User user)
        {
            try
            {
                ...
                Alert($"{user.Info.Nickname} disconnected");
        ...
        public bool Disconnect(User user, IPEndPoint currentEnd)
        {
